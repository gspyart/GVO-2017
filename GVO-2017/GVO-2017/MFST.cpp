...
namespace MFST
{
	...
	Mfst::Mfst(LexTable::LexTable plex, GRB::Greibach pgrebach)//(результат работы лексического анализатора, грамматика Грейбах)
	{
	...
	Mfst::RC_STEP Mfst::step(const output::OUTPUT &log) //выполнить шаг автомата
	{
	...
	bool Mfst::savestate(const output::OUTPUT &log) //сохранить состояние автомата
	{
	...
	bool Mfst::reststate(const output::OUTPUT &log) //восстановить состояние автомата
	{
		bool rc = false;
		MfstState state;
	...
	bool Mfst::start(const output::OUTPUT &log)
	{
		bool rc = false;
		...
		case LENTA_END:         MFST_TRACE4(log, "------>LENTA_END")
			...
			*log.txt << std::setw(4) << std::left << 0 << ":всего строк " << lenta_size << ", синтаксический анализ выполнен без ошибок " << std::endl;
			rc = true;
			break;
		case NS_NORULE:         MFST_TRACE4(log, "------>NS_NORULE")
			*log.txt << "-------------------------------------------------------------------------------------" << std::endl;
			*log.txt << getDiagnosis(0, buf) << std::endl;
			*log.txt << getDiagnosis(1, buf) << std::endl;
			*log.txt << getDiagnosis(2, buf) << std::endl;
			break;
