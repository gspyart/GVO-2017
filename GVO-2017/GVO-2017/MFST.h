...
namespace MFST
{
	struct ParseTree                  // дерево разбора
	{
		short* nrule;
		short* nrulechain;
		int    size;
	...
	struct Mfst					//магазинный автомат	
	{
		...
		LexTable::LexTable lex;					//результат работы лексического анализатора
		MFSTSTSTACK st;					//стек автомата		
		std::stack<MfstState> storestate;	//стек для сохранения состояний
		ParseTree PT;
		Mfst();
		Mfst(
			LexTable::LexTable plex,			//результат работы лексического анализатора
		...
		bool savestate(const output::OUTPUT &log);					//сохранить состояние автомата
		bool reststate(const output::OUTPUT &log);					//восстановить состояние автомата
		bool push_chain(					//поместить уепочку правила в стек
			GRB::Rule::Chain chain				//цепочка правила
		);
		RC_STEP step(const output::OUTPUT &log);			//выполнить шаг автомата
		bool start(const output::OUTPUT &log);			//запустить автомат
		bool savediagnois(
			RC_STEP pprc_step			//код завершения шага
		);
		void printrules(const output::OUTPUT &log);		//вывести последовательность правил
